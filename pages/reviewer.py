...
if not pending_contents:
    ...
    if st.button("Submit Decision", use_container_width=True):
        if not reason.strip():
            ...
            try:
                # Submit decision → update content + audit log
                submit_decision(
                    content=selected,
                ...
                log_event_google(
                    ...
                    severity=selected["validation_result"]["final_severity"],
                    triggered_rules=", ".join([r["rule_id"] for r in selected["validation_result"].get("triggered_rules", [])]),
                    decision=decision,  # REVISION_REQUIRED / APPROVED
                ...
                #    event_type="SUBMIT_DECISION",
                #    object_type="DECISION",
                #    object_id=selected.get("content_id", ""),
                #    content_excerpt=selected.get("content", "")[:200],
                #    decision=decision,
                #    severity=severity,
                #    triggered_rules=", ".join([r.get("rule_id","") for r in triggered_rules])
                #)
...
if audit_log:
    for i, record in enumerate(reversed(audit_log), 1):
        with st.expander(
            f"Record #{len(audit_log)-i+1} — {record.get('decision','N/A')} | "
